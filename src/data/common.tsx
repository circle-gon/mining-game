...
export type ResetButton<T extends ResetButtonOptions> = Replace<
  Clickable<T>,
  {
    resetDescription: GetComputableTypeWithDefault<
      T["resetDescription"],
      Ref<string>
    >;
...
>(optionsFunc: OptionsFunc<T>): ResetButton<T> {
  return createClickable(() => {
    ...
    if (resetButton.resetDescription == null) {
      resetButton.resetDescription = computed(() =>
        Decimal.lt(resetButton.conversion.gainResource.value, 1e3)
          ? "Reset for "
          : ""
      );
    } else {
          ...
          <b>
            {displayResource(
              ...
              Decimal.max(
                ...
                unref(
                  resetButton.minimumGain as ProcessedComputable<DecimalSource>
                )
              )
            )}
          </b>{" "}
      ...
      const header = (
        ...
        >
          <span
            class={
              "modifier-toggle" +
              (unref(collapsed.value[i]) ? " collapsed" : "")
            }
          >
            ▼
          </span>
