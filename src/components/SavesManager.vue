      ...
      <div class="modal-footer">
        ...
        <div class="field">
          ...
          <div class="field-buttons">
            <button class="button" @click="openSave(newSave().id)">
              New Game
            </button>
            <Select
...
import { getUniqueID, loadSave, newSave, save, decodeSave } from "util/save";
...
export type LoadablePlayerData = Omit<Partial<PlayerData>, "id"> & {
  id: string;
  error?: unknown;
};
...
watch(saveToImport, (importedSave) => {
  if (importedSave) {
    nextTick(() => {
      try {
        const playerData = JSON.parse(decodeSave(importedSave));
...
const cachedSaves = shallowReactive<
  Record<string, LoadablePlayerData | undefined>
>({});
    ...
    if (save == null) {
      ...
      try {
        try {
          ...
          importingFailed.value = true;
          cachedSaves[id] = {
            error: "Unable to determine preset encoding",
            id,
          };
          // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
          return cachedSaves[id]!;
        }
        cachedSaves[id] = { ...JSON.parse(save as string), id };
      } catch (error) {
...
const saves = computed(() =>
  settings.saves.reduce(
    (acc: Record<string, LoadablePlayerData>, curr: string) => {
...
function exportSave(id: string) {
  ...
  switch (projInfo.exportEncoding) {
    default:
      console.warn(
        `Unknown save encoding: ${projInfo.exportEncoding}. Defaulting to lz`
      );
    case "lz":
