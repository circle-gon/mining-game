...
export function persistent<T extends State>(
  ...
  persistent[DefaultValue] = isRef(defaultValue)
    ? defaultValue.value
    : defaultValue;
...
export function deletePersistent(persistent: Persistent) {
  if (addingLayers.length === 0) {
    console.warn(
      "Deleting a persistent ref outside of a layer. Ignoring...",
      persistent
    );
  } else {
...
globalBus.on(
  "addLayer",
  (layer: GenericLayer, saveData: Record<string, unknown>) => {
      ...
      Object.keys(obj).forEach((key) => {
        ...
        if (value && typeof value === "object") {
          if (PersistentState in value) {
            ...
            const persistentState = path.reduce<Record<string, unknown>>(
              (acc, curr) => {
                if (!(curr in acc)) {
                  acc[curr] = {};
                }
            ...
            if (isReactive(persistentState)) {
              ...
              if (savedValue != null) {
                ...
                (persistentState[key] as Ref<unknown>).value = (
                  value as Persistent
                )[DefaultValue];
              }
            }
