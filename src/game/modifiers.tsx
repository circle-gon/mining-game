...
export function createAdditiveModifier<T extends AdditiveModifierOptions>(
  ...
  return createLazyProxy(() => {
    ...
    const processedEnabled =
      enabled == null ? undefined : convertComputable(enabled);
    return {
      apply: (gain: DecimalSource) => Decimal.add(gain, unref(processedAddend)),
      revert: (gain: DecimalSource) =>
        Decimal.sub(gain, unref(processedAddend)),
  ...
  return createLazyProxy(() => {
    ...
    return {
      apply: (gain: DecimalSource) =>
        Decimal.times(gain, unref(processedMultiplier)),
      revert: (gain: DecimalSource) =>
        Decimal.div(gain, unref(processedMultiplier)),
      enabled: processedEnabled,
      description:
        description == null
          ? undefined
  ...
  return createLazyProxy(() => {
    ...
    return {
      ...
      revert: modifiers.every((m) => m.revert != null)
        ...
        : undefined,
      enabled: computed(
        () => modifiers.filter((m) => unref(m.enabled) !== false).length > 0
      ),
      description: jsx(() => (
        <>
          {(
