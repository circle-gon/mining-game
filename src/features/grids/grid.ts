...
export type CellComputable<T> =
  | Computable<T>
  | ((id: string | number, state: State) => T);
...
function getGridHandler(
  grid: GenericGrid
): ProxyHandler<Record<string | number, GridCell>> {
  const keys = computed(() => {
    const keys = [];
  ...
  return {
    ...
    get(target, key, receiver): any {
      ...
      if (isFunction(prop)) {
        if (!(key in cache)) {
          cache[key] = computed(() =>
            prop.call(receiver, id, target.getState(id))
          );
        }
