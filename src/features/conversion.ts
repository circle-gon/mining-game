...
export type Conversion<T extends ConversionOptions> = Replace<
  T & BaseConversion,
  {
    currentGain: GetComputableTypeWithDefault<
      T["currentGain"],
      Ref<DecimalSource>
    >;
    actualGain: GetComputableTypeWithDefault<
      T["actualGain"],
      Ref<DecimalSource>
    >;
    ...
    spend: undefined extends T["spend"]
      ? (amountGained: DecimalSource) => void
      : T["spend"];
  ...
  return createLazyProxy(() => {
    ...
    if (conversion.currentAt == null) {
      conversion.currentAt = computed(() => {
        let current = conversion.scaling.currentAt(
          conversion as GenericConversion
        );
    ...
    if (conversion.convert == null) {
      conversion.convert = function () {
        const amountGained = unref(
          (conversion as GenericConversion).currentGain
        );
...
export function createLinearScaling(
  ...
  return {
    ...
    nextAt(conversion) {
      let next: DecimalSource = Decimal.add(
        unref(conversion.currentGain),
        1
      ).floor();
      if (conversion.gainModifier) {
...
export function setupPassiveGeneration(
  ...
  layer.on("preUpdate", (diff) => {
    ...
    if (Decimal.neq(currRate, 0)) {
      conversion.gainResource.value = Decimal.add(
        conversion.gainResource.value,
        Decimal.times(currRate, diff).times(
          Decimal.ceil(unref(conversion.actualGain))
        )
...
export function softcap(
  ...
  if (Decimal.lte(value, cap)) {
    ...
    return Decimal.pow(value, power).times(
      Decimal.pow(cap, Decimal.sub(1, power))
    );
  }
}
...
export function addHardcap(
  ...
  return {
    ...scaling,
    currentGain: (conversion) =>
      Decimal.min(scaling.currentGain(conversion), unref(cap)),
  };
}
