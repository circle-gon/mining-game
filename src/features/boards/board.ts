...
export interface NodeTypeOptions {
  ...
  canAccept?:
    | boolean
    | Ref<boolean>
    | ((node: BoardNode, otherNode: BoardNode) => boolean);
...
export type NodeType<T extends NodeTypeOptions> = Replace<
  T & BaseNodeType,
  {
    ...
    progressDisplay: GetComputableTypeWithDefault<
      T["progressDisplay"],
...
export function createBoard<T extends BoardOptions>(
  ...
  return createLazyProxy(() => {
    ...
    for (const type in board.types) {
      const nodeType: NodeTypeOptions & Partial<BaseNodeType> =
        board.types[type];
...
export function getNodeProperty<T>(
  ...
  return isFunction<T, [BoardNode], Computable<T>>(property)
    ? property(node)
    : unref(property);
}
...
globalBus.on("addLayer", (layer) => {
  const boards: GenericBoard[] = findFeatures(
    layer,
    BoardType
  ) as GenericBoard[];
