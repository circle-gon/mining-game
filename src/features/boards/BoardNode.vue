    ...
    >
      ...
      <g v-else-if="shape === Shape.Diamond" transform="rotate(45, 0, 0)">
        ...
        <rect
          ...
          :width="size * sqrtTwo"
          :height="size * sqrtTwo"
          :transform="`translate(${(-size * sqrtTwo) / 2}, ${
            (-size * sqrtTwo) / 2
          })`"
          :fill="fillColor"
          :stroke="outlineColor"
...
const isSelected = computed(
  () => unref(props.selectedNode) === unref(props.node)
);
...
const shape = computed(() =>
  getNodeProperty(props.nodeType.value.shape, unref(props.node))
);
const title = computed(() =>
  getNodeProperty(props.nodeType.value.title, unref(props.node))
);
const label = computed(() =>
  getNodeProperty(props.nodeType.value.label, unref(props.node))
);
const size = computed(() =>
  getNodeProperty(props.nodeType.value.size, unref(props.node))
);
const progress = computed(
  () => getNodeProperty(props.nodeType.value.progress, unref(props.node)) || 0
);
const backgroundColor = computed(
  () => themes[settings.theme].variables["--background"]
);
