  ...
  return createLazyProxy(() => {
    ...
    challenge.toggle = function () {
      ...
      if (genericChallenge.active.value) {
        if (
          ...
          let completions: boolean | DecimalSource = unref(
    ...
    if (challenge.canComplete == null) {
      challenge.canComplete = computed(() => {
        ...
        return Decimal.gte(
          genericChallenge.resource.value,
          unref(genericChallenge.goal)
        );
      });
    }
    ...
    if (challenge.reset != null) {
      globalBus.on("reset", (currentReset) => {
        if (
          currentReset === challenge.reset &&
          (challenge.active as Ref<boolean>).value
        ) {
...
export function isAnyChallengeActive(
  ...
  return computed(
    () => (challenges as Ref<GenericChallenge | undefined>).value != null
  );
}
