...
const nodes = inject(
  NodesInjectionKey,
  ref<Record<string, FeatureNode | undefined>>({})
);
const outerBoundingRect = inject(
  BoundsInjectionKey,
  ref<DOMRect | undefined>(undefined)
);
const boundingRect = ref<DOMRect | undefined>(
  resizeListener.value?.getBoundingClientRect()
);
watch(
  outerBoundingRect,
...
const validLinks = computed(() => {
  ...
  return (
    links.value?.filter(
      (link) => n[link.startNode.id]?.rect && n[link.startNode.id]?.rect
    ) ?? []
  );
});
</script>
