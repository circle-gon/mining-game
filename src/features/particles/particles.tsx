...
export function createParticles<T extends ParticlesOptions>(
  ...
  return createLazyProxy(() => {
    const particles =
      optionsFunc?.() ?? ({} as ReturnType<NonNullable<typeof optionsFunc>>);
    ...
    particles.addEmitter = (config: EmitterConfigV3): Promise<Emitter> => {
      ...
      if (genericParticles.app.value) {
        return Promise.resolve(
          new Emitter(genericParticles.app.value.stage, config)
        );
      }
    ...
    function onInit(app: Application) {
      ...
      emittersToAdd.forEach(({ resolve, config }) =>
        resolve(new Emitter(app.stage, config))
      );
      emittersToAdd = [];
    }

    particles.onContainerResized =
      particles.onContainerResized?.bind(particles);
